...
DualShock4_Info_TypeDef ds4Info;

void __attribute__((weak)) usr_dualshock4_handle(const DualShock4_Info_TypeDef *ds4Info)
{
}

void PS4_OnInitBTHID(void *phost)
{
  uint8_t buf[2];
  buf[0] = 0x43; // HID BT Get_report (0x40) | Report Type (Feature 0x03)
  buf[1] = 0x02; // Report ID

  btd_l2cap_command(phost, bthid.service.hci_handle, buf, 2, bthid.control_scid[0], bthid.control_scid[1]);
}

void PS4_ParseBTHIDData(uint8_t len, uint8_t *buf)
{
  if (buf[0] == 0x01)
  {
  }
  ...
  {
    dualshock4_report_parser(buf + 2, &ds4Info);

    usr_dualshock4_handle(&ds4Info);
  }
}
  ...
  if (checkHciHandle(l2capinbuf, bthid.service.hci_handle)) // acl_handle_ok
  {
    ...
    else if (l2capinbuf[6] == bthid.interrupt_dcid[0] && l2capinbuf[7] == bthid.interrupt_dcid[1]) // l2cap_interrupt
    {
      if (l2capinbuf[8] == 0xA1) // HID BT DATA (0xA0) | Report Type (Input 0x01)
      {
          ...
          PS4_ParseBTHIDData((uint8_t)(length - 1), &l2capinbuf[9]);
